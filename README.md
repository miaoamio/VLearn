...
| 需求场景 | 安装命令 |
|---------|---------|
| 飞书头像组件（可点击跳转飞书 Profile） | `pnpm add @tod-m/materials less` |
| 动画效果 | `pnpm add framer-motion` |
| 企业级组件（Table、TreeSelect 等） | `pnpm add @arco-design/web-react` |
| HTTP 请求库 | `pnpm add axios` |
| 状态管理 | `pnpm add zustand` |
| Markdown 渲染 | `pnpm add react-markdown` |
| 拖拽排序 | `pnpm add @dnd-kit/core @dnd-kit/sortable` |

## 开发规范与框架约束

> 本章节面向 AI Agent 和开发者，提供项目开发的完整上下文。本项目使用模块化文档架构，请根据任务类型选择阅读，阅读完相关文档后，严格遵循框架约束进行开发。
